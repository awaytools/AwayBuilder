package awaybuilder.view.components.controls
{
	...
	public class DroppableLightsList extends List
	{
		public function DroppableLightsList()
		{
			super();
		}
		...
		override protected function dragDropHandler(event:DragEvent):void
		{
			...
			for (i = 0; i < items.length; i++)
			{
				...
				var clonnedItem:LightVO = LightVO(item.item).clone() as LightVO;
				//				if (copyItems)
		...
		private function calculateDropLocation(event:DragEvent):DropLocation {
			...
			try 
			{
				...
				{
					var item:ScenegraphItemVO = items[i] as ScenegraphItemVO;
					var asset:LightVO = item.item as LightVO;
					if( !asset ) 
						return null;
