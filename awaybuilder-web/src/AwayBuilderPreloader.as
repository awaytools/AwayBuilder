package
{
	...
	public class AwayBuilderPreloader extends Sprite implements IPreloaderDisplay
	{
		 ...
		 */
		public function AwayBuilderPreloader() 
		{
			super();
		}
		...
		{       
			...
			// Determine the size
			var totalWidth:Number = Math.min(stageWidth - 10, 807);
			var totalHeight:Number = 17;
			var startX:Number = Math.round((stageWidth - totalWidth) / 2);
			var startY:Number = stageHeight-110;
			
			_barWidth = totalWidth - 8;
			...
			_barFrameSprite.x = _barSprite.x = startX + 4;
			_barFrameSprite.y = _barSprite.y = startY + 4;
			...
			this.addChild( text );
			text.x = startX + totalWidth - text.width;
			text.y = startY - 23;
			...
			versionText.width = text.textWidth+3;
			this.addChild( versionText );
			versionText.x = startX;
			versionText.y = startY - 23;
		}
		...
		{
			// TODO: Preloader must be removed when Scene is ready
...
{
	private var downloadProgressBar:AwayBuilderPreloader;
	 ...
	 */ 
	public function ErrorField(downloadProgressBar:AwayBuilderPreloader)
	{
		super();
